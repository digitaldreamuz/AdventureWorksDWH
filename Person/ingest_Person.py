...
PG_DATABASE = "raw"
PG_SCHEMA = "person"  # Target schema in PostgreSQL
...
for table in tables:
    try:
        ...
        sql_query = f"SELECT * FROM Person.{table}"
        df = pd.read_sql(sql_query, sql_conn)

        if df.empty:
            print(f"⚠️ Person.{table} is empty. Skipping...")
            continue
        ...
        print(f"✅ Person.{table} transferred successfully!")
    except Exception as e:
        print(f"❌ Error transferring Person.{table}: {e}")
