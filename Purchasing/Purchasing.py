...
PG_DATABASE = "raw"
PG_SCHEMA = "Purchasing"  # Target schema in PostgreSQL
...
try:
    ...
    print(f"📌 Tables to transfer from 'Purchasing' schema: {tables}")
except Exception as e:
    print(f"❌ Error fetching table names for 'Purchasing' schema: {e}")
    exit()

# Loop through each table and migrate data
for table in tables:
    try:
        print(f"🔄 Transferring table: Purchasing.{table}...")

        # Read table from SQL Server
        sql_query = f"SELECT * FROM Purchasing.{table}"
        df = pd.read_sql(sql_query, sql_conn)

        if df.empty:
            print(f"⚠️ Purchasing.{table} is empty. Skipping...")
            continue
        ...
        print(f"✅ Purchasing.{table} transferred successfully!")
    except Exception as e:
        print(f"❌ Error transferring Purchasing.{table}: {e}")

# Close SQL Server connection
sql_conn.close()
print("🎉 Data migration from 'Purchasing' schema completed successfully!")