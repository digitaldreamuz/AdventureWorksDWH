...
PG_DATABASE = "raw"
PG_SCHEMA = "Production"  # Target schema in PostgreSQL
...
try:
    ...
    print(f"📌 Tables to transfer from 'Production' schema: {tables}")
except Exception as e:
    print(f"❌ Error fetching table names for 'Production' schema: {e}")
    exit()

# Loop through each table and migrate data
for table in tables:
    try:
        print(f"🔄 Transferring table: Production.{table}...")

        # Read table from SQL Server
        sql_query = f"SELECT * FROM Production.{table}"
        df = pd.read_sql(sql_query, sql_conn)

        if df.empty:
            print(f"⚠️ Production.{table} is empty. Skipping...")
            continue
        ...
        print(f"✅ Production.{table} transferred successfully!")
    except Exception as e:
        print(f"❌ Error transferring Production.{table}: {e}")

# Close SQL Server connection
sql_conn.close()
print("🎉 Data migration from 'Person' schema completed successfully!")